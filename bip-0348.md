...
Only 32-byte and 0-byte keys are constrained. Similar to [BIP 342] unknown key types, for
other key lengths no signature verification is performed and it is considered
successful. We deviate from [BIP 342] in that a 0-byte key is punned to the internal key.
...
* If the public key size is zero, it is considered to be the taproot internal key as described in [BIP 341].
...
5. APO-style ([BIP 118]) Taproot internal key: Rather than unnecessarily removing the 0-length key type in this change, we use it as a stand-in for the taproot internal key.
...
`OP_CHECKSIGFROMSTACK` construction is 8 vBytes larger. If the internal key
can be used, the script `OP_CHECKTEMPLATEVERIFY OP_0 OP_CHECKSIGFROMSTACK` can
be used which is 8 vBytes smaller, reducing the efficiency difference.
